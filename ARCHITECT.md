...
**D-48 (post-v1.0 amendment — the approval gate decides on invocations, and refuses to guess a root):** two defects, both dogfound within an hour of the first session running the D-47 hooks, both the same shape D-47 named: the surface said one thing and the mechanism did another.

*Text is not an invocation.* The gate matched the raw command string to decide WHICH commands it gated, so `grep -rn 'cgel unblock' README.md` — a read — was denied and told to go collect a user's approval for a file read. This is verbatim the bug `cmdline.py` was written for ("a read of a command is not a run of it"); `command_guard` was routed through it in TASK-2 and this gate was not, and D-47's `_CGEL` anchors fixed only the false-NEGATIVE half (`-C` walking past the gate). Purpose now comes from each segment's resolved argv — and from nowhere else. The first cut of this fix kept the raw-text table as a co-equal fallback that could still extract purposes, roots and pins from an unreadable line, which recreated the disease at a new joint: two implementations of one predicate, obliged to agree forever. Four consecutive review rounds each found them disagreeing — a flag spelled with `=`, a subshell opener hiding argv[0], cgel invoked by path, a pin extracted from the wrong invocation. The table is deleted. What remains for a line `cmdline.analyze` cannot resolve is a TRIPWIRE: if the text so much as hints at a gated verb, the line is refused outright, with the remedy stated — run the verb as a plain single command. Text can refuse; it can never authorise, root, or bind. A redirection (`echo '...cgel unblock...' > note.md`) is therefore still stopped — the accepted cost, and the reason the skills say to use Edit, not a shell heredoc, for repo files — but no verdict is ever again taken from words.

*A root we cannot name.* D-47 kept the Bash guards cwd-rooted deliberately, and that trade stands. What did not stand is what was said about it. The gate rooted `cd elsewhere && cgel seal ...` at the SESSION's project anyway: measured, a seal of a scratch fixture wrote its approval row into `cgel-plugin`'s own ledger — an approval record filed against a repository that was never sealed, in a product whose thesis is that records mean something — and the next seal was denied because that key was spent on another project's digest. Where the session's own config said `approval_gate: off`, the seal of a project that had NOT turned the gate off was waved through: the one-flag bypass D-47 closed for `-C`, alive in `cd` form. The fix is not to model the shell. `cd /a ; cgel seal` and `cd /a || cgel seal` land in a directory no PreToolUse hook can know before the line runs, and `$VAR` is not opaque to the tokenizer, so cd-tracking would false-deny — and it would drag `cmdline.py` and the fail-closed `command_guard` into the blast radius to do it. Instead the existing rule is applied to the one case it had not reached (tombstone, `tests/test_approvals.py`): an approval-gated verb we cannot root is a verb we cannot gate, so a gated verb sharing a line with a `cd` is DENIED, remedy named — `cgel -C <dir>`, absolute, which needs no `cd` and is what the gate reads. An absolute `-C` pins the project, so it is allowed through a `cd`; a `cd` AFTER the verb cannot move it, so it is not denied.

*The one-flag bypass, still open, in the spelling nobody tried.* Holding the argv reader against the old text table in a test immediately surfaced a line the TEXT did not gate: `cgel -C. seal T-1 --digest …`. `-C` is a short option, so argparse takes its value attached, and that seal is legal and works — but every anchor demanded `=` or whitespace after the flag, so the line matched nothing at all, `seal` was never detected, and the gate stood aside. An unapproved seal, in the shipped 0.13.0, through the exact flag D-47 shipped the anchors to defend: it verified `-C .` and `--directory=.` and never tried `-C.`, and "verified: all seven gated verbs go DENY → ALLOW" was true of the two spellings it tested. The durable fix is that no anchor decides anymore: the decider parses the attached spelling out of argv like any other, and the tripwire has no flag anchoring to get wrong — `cgel` followed anywhere on the line by a gated verb trips it, in every `-C` spelling at once. The general lesson is the one D-47 keeps relearning: an anchor is only as good as the spellings it was tried against, and the way to find the untried one is to hold two independent implementations against each other rather than to re-read one.

*One line was one verdict.* The audit that followed found the hole four review rounds had missed, in both the shipped gate and the draft: enforcement was per LINE. One purpose string, the FIRST digest on the line, one root, one vouch — so `cgel seal T1 --digest APPROVED && cgel seal T2 --digest NEVERAPPROVED` was vouched on the strength of the first digest alone, and the harness ran the unapproved seal unprompted. Reverse the order and it denied: enforcement that depends on segment order is not enforcement. The gate now walks the line's gated invocations one by one — each finds its own approval, its own `-C`, its own project config (a sibling's `approval_gate: off` exempts nobody but itself) — and consumption is two-phase: nothing is spent until every invocation has held, so a deny costs no approval. The vouch that suppresses the permission prompt is emitted only when every gated invocation was individually satisfied and the user read everything the line does (all-cgel line, or an approval quoting the exact command).

*The floor, stated.* This gate's trust class was always same-principal and tamper-evident (D-38); the redesign states its parsing floor to match: literal spellings. A deliberately mangled invocation — `cgel se"al"`, a digest built by substitution — is outside the class on every path, before and after; what the design guarantees is that no HONEST spelling of a gated verb slips through reading, and that nothing unreadable is ever guessed about. The attempt, as always, stays visible in the transcript.

*And the notice was false in the other direction.* SessionStart promised a monorepo session that the Bash guards "are NOT active here" and the README said "nothing is watching the command line". A gated verb there does not stand aside — it fails closed, and it did so blaming `cgel` for "naming a directory that is not a CGEL project" when no `-C` had been typed and the project existed one level down. Both halves were wrong: the guard is real, its answer is always no, and the remedy was never stated. Ungated verbs stand aside (that much was true); gated ones deny and now say how to address the project. D-47 wrote that the residual "is stated in the README and announced by SessionStart rather than left as an absence" — it was announced, and what was announced was not what the code did.

**D-49 (post-v1.0 amendment — three review rules for the practices the bar was missing):** the built-in set grows from four rules to seven, adopting three Claude Code review habits the production bar did not encode. `CGEL-CORRECT-1` (blocking) puts a correctness pass — null/None deref, unchecked error, off-by-one, resource leak, broken invariant — on the diff the verifier already reads at close, so reviewing one's own edge cases costs no extra agent and no manual `/code-review`. `CGEL-ROOT-1` (blocking) makes a fix cure the cause rather than paper over it; the clause moves out of `CGEL-DEBT-1` (which stays advisory, D-46) so root-cause papering is not named by two rules at once — the very duplication `CGEL-DEBT-1` forbids. `CGEL-TEST-1` (advisory) asks that new behavior ship with a test that would fail without it. The D-46 split still holds: `CGEL-CORRECT-1` sits on its checkable side (a defect is a line you point at), and `CGEL-ROOT-1` is the one block taken off that principle by choice — the user asked for exactly it ("fix the root cause, no patchwork"), and the off-switch risk D-46 names is bounded by scoping the rule to root-cause papering alone, by the same-id `docs/standards` override, and by `builtin_rules: off`. The verifier gains a concrete duty per new rule and its output budget rises from ~1500 to ~2800 tokens, capped to the few most serious findings per rule, because seven rules cannot share the old cap without starving IMPACT-1's greps. No CLI, gate, or schema change: `_semantic_requirement` and `cmd_semantic` already read each rule's `Blocking:` field from `builtin.md`, so blocking is data and the whole amendment is rules, prose, and tests.

**D-50 (post-v1.0 amendment — a tailored CLAUDE.md at onboarding, without a hole in the gate):** the plugin now writes a repo-specific CLAUDE.md when a fresh project is first set up. The enabling change is one lifecycle-gated exemption in `contract_gate`: root `CLAUDE.md` and `CLAUDE.local.md` (a new `ROOT_MEMORY_FILES` set in `cgel_common`) are writable only when NO task governs the repo — the onboarding window, before the first seal. Three alternatives were rejected in review. A static template from `cgel init` was not what was asked for (a *tailored* file). Adding the files to `DRAFT_EXEMPT_PATTERNS` was unsafe: that check runs before the forbidden veto and ignores lifecycle, so CLAUDE.md would be writable mid-task, unscoped and unrecorded — and CLAUDE.md is model-loaded memory, so that is self-instruction smuggling. Auto-sealing a dedicated CLAUDE.md task was rejected because seal is approval-gated on every invocation, forcing a full ceremony to write a memory file. The exemption is deliberately narrow: the moment any task is SEALED/ACTIVE/BLOCKED, CLAUDE.md follows normal scope like any file; a nested `sub/CLAUDE.md` is application content and stays gated; `.claude/CLAUDE.md` stays a governance path. It is kept OUT of the governance bundle, so a CLAUDE.md written at onboarding never becomes part of the sealed measure. The onboarding skill instructs the model to read the codebase and write a concise CLAUDE.md; that authorship is GUIDANCE_ONLY, while the exemption condition is HARD_ENFORCED.

**D-51 (post-v1.0 amendment — interview on unclear intent):** the task skill's intake now says, in the imperative, to INTERVIEW the user through AskUserQuestion when their intent is genuinely unclear — the goal, scope, acceptance bar, or choice of approach — before drafting the contract, rather than guessing and encoding the guess. This strengthens the pre-existing "ask if genuinely ambiguous" line into the official Claude Code practice of interviewing for a spec, and pairs with the challenger (D-45): the challenger questions a design the user HAS stated, while the interview surfaces intent the user has NOT yet stated. It is deliberately gated to real ambiguity — a plain request gets no interview — because a tool that asks when the answer is obvious trains the user to click through, the same failure the seal ceremony guards against. GUIDANCE_ONLY: prose in the skill, pinned by a test, never an enforced gate.

**D-52 (post-v1.0 amendment — the approval token is printed, the binding floor is not lowered):** transcripts of real v0.15.0 sessions showed users approving one seal twice, and occasionally three times. The cause was a split between the two halves of the ceremony: `approval_gate` binds a seal to the first 12 hex characters of the digest, matched as a literal substring of the approved question, but `cgel summary` printed only the full 64-hex digest — so the model chose its own truncation, an 8-character guess is not a substring of the 12-character token, and the seal was denied for a contract the user had already approved. The fix is on the ASKING side: `cgel summary` now prints the exact token to paste, the task skill says to copy it verbatim, and the seal deny names the near-miss when an approved question carried a shorter prefix of this same digest. The obvious alternative — letting the gate accept any prefix of >= 8 hex — was considered and REJECTED: 32 bits is grindable in a shell loop, so an approval of a benign contract could be made to bind a substituted one carrying a protected capability, and the substitution would look valid in the transcript rather than being evident. `_DIGEST_SHAPE`'s `{8,64}` is an argv shape sniff deciding whether anything bindable is present, never a declared binding floor. Widening a human consent anchor to work around a message the tool never printed is the wrong trade; if paraphrase persists, raise the DISPLAYED length, never lower the bind. The same reasoning keeps `approvals.find_approval` free of a `matcher=` callable — a second implementation of the binding predicate would violate this module's one-decider rule, so prefix awareness informs only the wording of a refusal. Text can refuse; it can never authorise.

**D-53 (post-v1.0 amendment — a rule for prose a human is meant to read):** the built-in set grows from seven rules to eight with `CGEL-CONCISE-1` (advisory), because the product had a bar for code and none for the documents, reports and summaries a change writes. The user's complaint was not length: it was that output arrived restating points already made, explaining background nobody asked for, and narrating the work where the result belonged — so it had to be edited before it could be forwarded to anyone. The rule names those failures and sets the test as forwardable-unedited, explicitly refusing a word count as the charge (D-54 later adds two more to the list). Its boundary against `CGEL-COMMENT-1` is drawn on the artifact, not the quality: comments stay COMMENT-1's, prose written for a reader is CONCISE-1's, so the two do not name one failure twice — the duplication `CGEL-DEBT-1` forbids and D-49 already had to untangle once between DEBT-1 and ROOT-1. It advises rather than blocks, holding the D-46 line for taste unchanged. The same bar is stated for session output in the task skill, where it cannot be enforced by a verifier at all — a rule that judged only the diff would leave the surface the user actually complained about ungoverned. Verifier budget rises ~2800 → ~3000 tokens for the eighth duty. No CLI, gate or schema change: blocking is data read from `builtin.md`, so the amendment is rules, prose and tests.

**D-54 (post-v1.0 amendment — prose leads with the action):** `CGEL-CONCISE-1` grows from three named failures to five, adding prose that puts an approach or a wind-up ahead of the thing the reader has to do, and ordered steps run together as a paragraph. D-53 governed what may be *said*; a paragraph can pass every one of its three charges — no restatement, no unrequested background, no narration — and still make the reader hunt three sentences in for the command. So the amendment governs *order*: the command, path or `file:line` first, the reasoning after, and a numbered list where steps must happen in sequence. Session output gets the same treatment where the verifier cannot reach it — the task skill's "Talking to the user" names the anti-patterns concretely ("great question", "hope this helps") rather than paraphrasing them, because a paraphrase of a tic does not survive rewording; the loop skill fixes how a failing check is reported (the check and the error it printed, never dressed in alarm); the attest skill fixes the closing report's order (what changed, what proved it, what is left). Two carve-outs are written into the rule itself, because an action-first mandate over-applied is the same failure pointing the other way: prose asked to explain, teach or walk through something is judged on the same forwardable bar, and there the explanation IS the answer; and a destructive step is still confirmed before it runs however terse the exchange. The origin was a request to vendor a third-party skill (`github.com/ayghri/i-have-adhd`, MIT) into the marketplace, and that was REJECTED in intake: several of its rules contradict guidance CGEL already ships — "restate context each turn" against the task skill's "do not restate the request", and "give specific time estimates" against a product whose whole claim is that it does not assert what it has not measured — so a second always-on style skill would have competed with `CGEL-CONCISE-1` rather than reinforced it, at the cost of an attribution obligation for something a user can install from its own marketplace in one command. Only the two ideas CGEL genuinely lacked were taken. No CLI, gate or schema change, and no change to the blocking/advisory split: the amendment is rule text, skill prose, the verifier's duty line, the README table and tests.

**D-55 (post-v1.0 amendment — a roadmap that is planning material, never the measure):** long-horizon work kept dying in transcripts: users who asked for a roadmap doc and steered multi-session efforts against it were working around a product that had no home for "later". The amendment adds `cgel roadmap add/list/done/show` over `.cgel/roadmap.json` — entries carry a kind (`idea`, `milestone`, `verification-recipe`), and `verification-recipe` exists because the other thing that died in transcripts was the user's explanation of how to PROVE the project works end-to-end (build, bring-up, which API to hit, what the database should show); recorded, it is meant to be turned into registered checks between tasks, so e2e proof becomes evidence. Three shapes were rejected on review. A repo-root markdown pulled into each task's `scope.allowed` (the first proposal) works but puts planning behind the seal ceremony. An always-writable exemption in `contract_gate` is the D-50 hazard again — an unscoped, unrecorded write path punched through the gate. `.task/roadmap.json` fails on trust properties: `.task/` is deletable scratch the model may Edit freely, so a plan there can vanish or drift with no record. The CLI-verb route needs **zero hook changes**: contract_gate is PreToolUse `Edit|Write` and never sees the CLI's own file I/O, command_guard passes `cgel` invocations, and `.cgel/**` keeps direct edits behind `modify-governance` — so the verb is the only door, exactly like the registry. The one load-bearing line is the bundle exclusion: `governance_bundle` skips `ROADMAP_REL_PATH` hardcoded (not via config `bundle_exclude`, which a repo might lack) and unconditionally across bundle schemas (the file is created only by a verb that postdates every schema, so no sealed measure can legitimately contain it) — without it, every recorded idea flips every open task to BLOCKED. Unlike the registry there is no freeze window, so a present-but-corrupt file makes write verbs refuse rather than rewrite entries the user recorded. A merge of `.task/` into `.cgel/` was requested alongside and declined with the user's agreement: the two directories have opposite trust properties on purpose, and the itch — one native place — is satisfied by the roadmap living under `.cgel/`. The planner/executor split shipped as skill prose (GUIDANCE_ONLY): the worker was already pinned to a faster model with no Bash (D-44), every edit passes the same gate whichever agent makes it, and a hook cannot attribute a tool call to a subagent — a gate that claimed otherwise would be a D-46 surface asserting what the mechanism cannot keep.

**D-56 (post-v1.0 amendment — a cleanup sweep in the closing ceremony):** long tasks accumulate litter — comments that stopped matching the code three iterations ago, helpers nothing calls, debug leftovers — and the product's only answer was CGEL-COMMENT-1/CGEL-DEBT-1 advising at close, after the fact and non-blocking, which real use showed is too late: by then the litter ships. The attest skill now opens with a sweep, before any verifier runs, bounded twice: only the files `git diff HEAD` already shows (the user's own constraint — the pass must not be able to impact anything else — encoded as the diff being the boundary, with the edit gate as backstop), and behavior-preserving deletions only (a removal that changes behavior is a change, and takes a normal iteration). The sweep runs as its own iteration on fresh evidence when there is anything to delete, and is skipped when there is not — a ceremony with nothing to do is noise. Placement before the verifier is load-bearing, not stylistic: cleanup after `cgel semantic record` stales the findings and forces a second verifier run every time, while a sweep done first means the review judges the diff that actually ships. A hard-enforced cleanup gate was rejected on the D-46 line — whether a comment is redundant is a judgment no mechanism can check, so the pass is GUIDANCE_ONLY, pinned by tests, and the advisory rules remain the judge of whether it worked. No CLI, gate or schema change: the amendment is one skill section, the decision record, and the pins.
